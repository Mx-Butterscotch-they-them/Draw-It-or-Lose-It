...
public class GameAuthApplication extends Application<Configuration> {
	...
	public void run(Configuration c, Environment e) throws Exception 
	{
		...
		final Client client = new JerseyClientBuilder(e).build("game-auth-client");

		// create controller(s) that need the client
		final GameUserRESTController DemoRESTClient = new GameUserRESTController(e.getValidator());
		final RESTClientController restClient = new RESTClientController(client);
		
		
		//register controllers
		e.jersey().register(DemoRESTClient);
		e.jersey().register(restClient);
